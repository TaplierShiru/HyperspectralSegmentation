		...
		for j in range(n_2):
			with torch.no_grad():
				input_tensors_list = cut_into_parts_model_input(
					arr, h_parts=512//8, w_parts=512//8, h_win=8, w_win=8 
				)
				final_input_tensor = torch.cat(input_tensors_list, dim=0)
				pred_batch = net(final_input_tensor)
				final_mask = merge_parts_into_single_mask(
			        preds=pred_batch, shape=(1, 17, 512, 512), h_parts=512//8, 
