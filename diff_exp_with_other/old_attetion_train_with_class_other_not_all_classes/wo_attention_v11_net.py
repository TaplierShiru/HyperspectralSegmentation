...
#    { "cut_window": (64, 64),   "batch_size": 128,   "lr": 1e-3, "max_epochs": 10  },  # Max: 128
    { 
        ...
        "max_epochs": 350,  "epoch_list": None, "lr_list": None,
...
def start_exp(
    ...
    name_exp = f"WO Attention moduleV11 with other cut_window={cut_window} (run {number_of_exp})" +\
               f"change_lr_dyn | {max_epochs}ep | PCA. | RustamPreprocess(k=1) | CE " +\
               f"cosine(t_0={T_0},t_mul={T_mult}) lr={lr} | arch_type={arch_type} | batch_size={batch_size}"
    comet_exp.set_name(name_exp)
    comet_exp.add_tag('wo_attention_module_with_other')
