...
PREFIX_INFO_PATH = f'/home/rustam/hyperspecter_segmentation/danil_cave/kfolds_data_with_other/{KFOLD_NAME}/data'
...
class NnModel(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        pred_big_tensor = create_big_image(torch.clone(pred_tensor), single_elem_shape=[1, NUM_CLASSES, 512, 512])
    ...
    def test_epoch_end(self, outputs):
        print('Performe test with data n=', len(outputs))
        ...
        pred_tensor, target_tensor = collect_prediction_and_target(outputs, self, cut_window=self.cut_window)
        pred_as_mask = take_pred_masks(pred_tensor)
        pred_big_tensor = create_big_image(torch.clone(pred_tensor), single_elem_shape=[1, NUM_CLASSES, 512, 512])
        target_big_tensor = create_big_image(torch.clone(target_tensor), single_elem_shape=[1, 512, 512])
        ...
        for batch_idx, (target_s, pred_s) in enumerate(zip(target_tensor, pred_as_mask)):
            if self.enable_image_logging:
                ...
                sns.heatmap(pred_s.cpu().detach().numpy(), ax=ax1, vmin=0, vmax=NUM_CLASSES)
                sns.heatmap(target_s.cpu().detach().numpy(), ax=ax2, vmin=0, vmax=NUM_CLASSES)
                img_name = f'test_temp_fig_{GPU_ID}.png'
                ...
                if self.experiment is not None and self.current_epoch != 0:
                    # For Comet logger
                    self.experiment.log_image(
                        img_name, name=f'test_{batch_idx}', 
                        overwrite=False, step=self.global_step
                    )
...
device = 'cuda:0'
pca_explained_variance = np.load(f'{PREFIX_INFO_PATH}/{KFOLD_NAME}_PcaExplainedVariance_.npy')
pca_mean = np.load(f'{PREFIX_INFO_PATH}/{KFOLD_NAME}_PcaMean.npy')
pca_components = np.load(f'{PREFIX_INFO_PATH}/{KFOLD_NAME}_PcaComponents.npy')
...
def preprocessing(imgs, masks, map_mask_to_class=False, split_size=256):
    with open(f'{PREFIX_INFO_PATH}/data_standartization_params_{KFOLD_NAME}.json', 'r') as f:
...
def start_exp(
    ...
    test_indices = np.load(f'{PREFIX_INFO_PATH}/{KFOLD_NAME}_indx_unknown_test.npy')
    val_indices = np.load(f'{PREFIX_INFO_PATH}/{KFOLD_NAME}_indx_test.npy')
    train_indices = np.load(f'{PREFIX_INFO_PATH}/{KFOLD_NAME}_indx_train.npy')
    ...
    net = return_net(arch_type=arch_type, in_f=17, out_f=NUM_CLASSES)
    net.eval()
    with torch.no_grad():
    ...
    name_exp = f"Attention moduleV11 with other cut_window={cut_window} (run {number_of_exp})" +\
               f"change_lr_dyn | {KFOLD_NAME} | {max_epochs}ep | PCA. | RustamPreprocess(k=1) | CE " +\
               f"cosine(t_0={T_0},t_mul={T_mult}) lr={lr} | arch_type={arch_type} | batch_size={batch_size}"
    comet_exp.set_name(name_exp)
    comet_exp.add_tag('attention_module_with_other')
