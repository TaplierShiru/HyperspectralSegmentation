...
#    { "cut_window": (64, 64),   "batch_size": 128,   "lr": 1e-3, "max_epochs": 10  },  # Max: 128
    { 
        "cut_window": (128, 128), "batch_size": 128, "lr": 6e-3, 
        "max_epochs": 450,  "epoch_list": [-1, 250, 400], "lr_list": [6e-3, 1e-3, 1e-4],
    },  # Max: 128
    { 
        "cut_window": (256, 256), "batch_size": 86, "lr": 4e-3, 
        "max_epochs": 500,  "epoch_list": [-1, 300, 400], "lr_list": [4e-3, 1e-3, 1e-4],
    },  # Max: 64
    { 
        "cut_window": (512, 512), "batch_size": 22, "lr": 4e-3, 
        "max_epochs": 700,  "epoch_list": [-1, 450, 600], "lr_list": [4e-3, 1e-3, 1e-4], 
    },  # Max: 22
]
...
def start_exp(
    ...
    name_exp = f"Erase test cut_window={cut_window} (run {number_of_exp})" +\
    ...
    comet_exp.add_tag('random_erase_test')
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=1/(run={number_of_exp})Erase_test_"+\
