...
class NnModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        # TODO: Change lr after some epoch
...
#    { "cut_window": (64, 64),   "batch_size": 128,   "lr": 1e-3, "max_epochs": 10  },  # Max: 128
    { 
        "cut_window": (128, 128), "batch_size": 128, "lr": 1e-3, 
        "max_epochs": 450,  "epoch_list": [-1, 150, 250], "lr_list": [1e-3, 1e-4, 1e-5],
    },  # Max: 128
    { 
        "cut_window": (256, 256), "batch_size": 86, "lr": 8e-4, 
        "max_epochs": 500,  "epoch_list": [-1, 200, 350], "lr_list": [8e-4, 8e-5, 8e-6],
    },  # Max: 64
    { 
        "cut_window": (512, 512), "batch_size": 22, "lr": 5e-4, 
        "max_epochs": 700,  "epoch_list": [-1, 300, 550], "lr_list": [5e-4, 5e-5, 5e-6], 
    },  # Max: 22
]
...
def start_exp(
    ...
    name_exp = f"MySuperNetLittleInput Segmentation cut_window={cut_window} (run {number_of_exp})"+\
               f" change_lr_dyn rerun=3 | LrCosine W weight decay | lower arch | {max_epochs}ep | Wo full PCA." +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=3/(run={number_of_exp})MySuperNetLittleInput Segmentation"+\
                f"  cut_window={cut_window} change_lr_dyn rerun=3"+\
                f"| _LrCosine W weight decay lower_" +\
                f"arch_{max_epochs}ep_Wo full PCA._RustamPreprocess(k=1)_CE"+\
