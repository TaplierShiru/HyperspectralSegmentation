...
# "max_epochs": 450,  "epoch_list": [-1, 250, 400], "lr_list": [6e-3, 1e-3, 1e-4],
    { 
        "cut_window": (3, 3), "batch_size": 8192, "lr": 6e-3, 
        "max_epochs": 5,  "epoch_list": None, "lr_list": None,
    },
    { 
        "cut_window": (5, 5), "batch_size": 8192, "lr": 6e-3, 
        "max_epochs": 5,  "epoch_list": None, "lr_list": None,
    },
    { 
        "cut_window": (7, 7), "batch_size": 8192, "lr": 6e-3, 
        "max_epochs": 5,  "epoch_list": None, "lr_list": None,
    },
    { 
        "cut_window": (9, 9), "batch_size": 8192, "lr": 6e-3, 
        "max_epochs": 5,  "epoch_list": None, "lr_list": None,
    },
    { 
        "cut_window": (11, 11), "batch_size": 8192, "lr": 6e-3, 
        "max_epochs": 5,  "epoch_list": None, "lr_list": None,
    },
    { 
        "cut_window": (1, 1), "batch_size": 8192, "lr": 6e-3, 
        "max_epochs": 5,  "epoch_list": None, "lr_list": None,
    },
]
...
def return_net(arch_type: str, in_f: int, out_f: int, cut_window: tuple):
    ...
    if arch_type == 'Unet':
        ...
        net = class_net(in_f, out_f, cut_window=cut_window)
    return net
...
def start_exp(
    ...
    net = return_net(arch_type=arch_type, in_f=17, out_f=17, cut_window=cut_window)
    net.eval()
    with torch.no_grad():
    ...
    name_exp = f"pixel_strategy cut_window={cut_window} (run {number_of_exp})" +\
    ...
    comet_exp.add_tag('pixel_strategy')
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=1/(run={number_of_exp})pixel_strategy_"+\
