...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.backbone = nn.Sequential(
            nn.Conv2d(in_f, 128, kernel_size=5, stride=1, padding=2, bias=False),
            # (N, 128, 8, 8)
            nn.BatchNorm2d(128),
            nn.ReLU(),
        ...
        # Output (N, 17, 8, 8)
        self.cam_module = CAM_Module(64)
        # Output (N, 17, 8, 8)
        self.pam_module = PAM_Module(64, dim_reduse=4)
        # Concat out_f and out_f = out_f * 2
        self.final_backbone = nn.Sequential(
            nn.Conv2d(64 * 3, out_f, kernel_size=1, stride=1, padding=0, bias=False),
...
#    { "cut_window": (64, 64),   "batch_size": 128,   "lr": 1e-3, "max_epochs": 10  },  # Max: 128
    { 
        "cut_window": (32, 32), "batch_size": 48, "lr": 1e-3, 
...
def start_exp(
    ...
    name_exp = f"Attention moduleV5 test cut_window={cut_window} (run {number_of_exp})" +\
