...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.backbone = nn.Sequential(
            nn.Conv2d(in_f, 128, kernel_size=3, stride=1, padding=1, bias=False),
            # (N, 128, 8, 8)
            nn.BatchNorm2d(128),
            nn.ReLU(),
            ...
            nn.Conv2d(64, out_f, kernel_size=3, stride=1, padding=1, bias=False),
            # (N, out_f, 8, 8)
        ...
        self.cam_train_output = nn.Sequential(
            nn.Conv2d(out_f, out_f, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(out_f),
            nn.ReLU(),
        )
        self.cam_final_output = nn.Conv2d(out_f, out_f, kernel_size=1, stride=1, padding=0, bias=False)
        # Output (N, 17, 8, 8)
        ...
        self.pam_final_output = nn.Conv2d(out_f, out_f, kernel_size=1, stride=1, padding=0, bias=False)
...
def start_exp(
    ...
    name_exp = f"Attention moduleV7 test cut_window={cut_window} (run {number_of_exp})" +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=7(v7)/(run={number_of_exp})Attention_module_test_"+\
