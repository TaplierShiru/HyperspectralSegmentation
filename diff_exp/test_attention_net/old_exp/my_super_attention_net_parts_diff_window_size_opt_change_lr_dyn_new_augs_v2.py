...
def collect_prediction_and_target(eval_loader, model, cut_window=(8, 8), image_shape=(512, 512), num_classes=17, divided_batch=4):
    target_list = []
    pred_list = []
...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.backbone = nn.Sequential(
            nn.Conv2d(in_f, 32, kernel_size=3, stride=1, padding=1, bias=False),
            # (N, 32, 8, 8)
            nn.BatchNorm2d(32),
            nn.ReLU(),
        ...
        # Output (N, 17, 8, 8)
        self.cam_module = CAM_Module(8)
        # Output (N, 17, 8, 8)
        self.pam_module = PAM_Module(8, dim_reduse=8)
        # Concat out_f and out_f = out_f * 2
        self.final_backbone = nn.Sequential(
            nn.Conv2d(8 * 2, out_f, kernel_size=1, stride=1, padding=0, bias=False),
...
def start_exp(
    ...
    name_exp = f"Attention moduleV2 test cut_window={cut_window} (run {number_of_exp})" +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=2(v2)/(run={number_of_exp})Attention_module_test_"+\
