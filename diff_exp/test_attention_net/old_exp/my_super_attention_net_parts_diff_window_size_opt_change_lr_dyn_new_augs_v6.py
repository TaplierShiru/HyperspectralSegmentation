...
class CAM_Module(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        energy_new = energy - energy_max_values # In paper - this stuff was swapped, 
                                                # i.e energy_max - energy
...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.final_backbone = nn.Sequential(
            nn.Conv2d(out_f * 3, out_f, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(out_f),
            nn.ReLU(),
            # Final conv
...
def start_exp(
    ...
    name_exp = f"Attention moduleV6 test cut_window={cut_window} (run {number_of_exp})" +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=6(v6)/(run={number_of_exp})Attention_module_test_"+\
