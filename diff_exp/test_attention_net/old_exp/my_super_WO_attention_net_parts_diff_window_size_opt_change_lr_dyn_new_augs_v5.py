...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.backbone = nn.Sequential(
            ...
            nn.Conv2d(64, out_f * 3, kernel_size=3, stride=1, padding=1, bias=False),
            # (N, out_f * 3, 8, 8)
            nn.BatchNorm2d(out_f * 3),
            nn.ReLU(),
...
def start_exp(
    ...
    name_exp = f"WO Attention moduleV5 test cut_window={cut_window} (run {number_of_exp})" +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=5(v5)/(run={number_of_exp})WO Attention_module_test_"+\
