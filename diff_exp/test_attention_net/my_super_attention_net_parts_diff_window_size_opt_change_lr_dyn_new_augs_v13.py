...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.backbone = nn.Sequential(
            # (N, in_f, 128, 128)
            nn.Conv2d(in_f, 64, kernel_size=7, stride=2, padding=3, bias=False),
            # (N, 64, 64, 64)
            nn.BatchNorm2d(64),
            nn.GELU(),
        ...
        # Output (N, 128, 32, 32)
        self.pam_module = PAM_Module(128, dim_reduse=4)
        ...
        self.final_backbone = nn.Sequential(
            ...
            nn.GELU(),
            # Final conv
            nn.Conv2d(64, out_f, kernel_size=1, stride=1, padding=0, bias=False),
        )
...
def start_exp(
    ...
    name_exp = f"Attention moduleV13 smaller-arch cut_window={cut_window} (run {number_of_exp})" +\
