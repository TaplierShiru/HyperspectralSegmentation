...
def init_weights(m):
    if isinstance(m, nn.Conv2d):
        torch.nn.init.kaiming_uniform_(m.weight, nonlinearity='GELU')
...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        self.backbone = nn.Sequential(
            # (N, in_f, 128, 128)
            nn.Conv2d(in_f, 32, kernel_size=7, stride=2, padding=3, bias=False),
            # (N, 32, 64, 64)
            nn.BatchNorm2d(32),
            nn.GELU(),
        ...
        # Output (N, 512, 32, 32)
        self.cam_module = CAM_Module(512)
        # Output (N, 512, 32, 32)
        self.pam_module = PAM_Module(512, dim_reduse=8)
        # Concat out_f and out_f = out_f * 2
        self.final_backbone = nn.Sequential(
            nn.Conv2d(512 * 3, 512, kernel_size=1, stride=1, padding=0, bias=False),
            # (N, 512, 32, 32)
            nn.BatchNorm2d(512),
            nn.GELU(),
            ...
            nn.ConvTranspose2d(128, 64, kernel_size=7, stride=2, padding=3, output_padding=1, bias=False),
            # (N, 64, 128, 128)
            nn.BatchNorm2d(64),
            nn.GELU(),
            
            nn.Conv2d(64, out_f, kernel_size=7, stride=1, padding=3, bias=False),
            # (N, out_f, 128, 128)
...
def start_exp(
    ...
    name_exp = f"Attention moduleV12 big-arch cut_window={cut_window} (run {number_of_exp})" +\
