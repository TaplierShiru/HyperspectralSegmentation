...
#
# https://github.com/PyTorchLightning/pytorch-lightning/issues/702
...
class NnModel(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        print('Each sample: ', len(outputs[0]))
        print(f"With in_shape={outputs[0][0].shape} AND val_shappe={outputs[0][0].shape}")
        ...
        self.log_dict(mean_iou_dict, sync_dist=True)
...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, *args):
        ...
        # Output (N, 17, 8, 8)
        self.pam_module = PAM_Module(out_f, dim_reduse=2)
        # Concat out_f and out_f = out_f * 2
        self.final_backbone = nn.Sequential(
            nn.Conv2d(out_f * 2, out_f, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(out_f),
            nn.ReLU(),
            # Final conv
    ...
    def forward(self, x):
        ...
        campam_x = cam_x + pam_x
        x = torch.cat((campam_x, x), dim=1)
...
def start_exp(
    ...
    if os.environ.get("NODE_RANK") is not None:
        os.environ['CUDA_VISIBLE_DEVICES'] = str(int(os.environ.get("NODE_RANK"))+1)
        node_rank = int(os.environ.get("NODE_RANK"))
        # os.environ['CUDA_VISIBLE_DEVICES'] = str(os.environ.get("NODE_RANK"))
    ...
    name_exp = f"Attention module test cut_window={cut_window} (run {number_of_exp})" +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/rerun=1/(run={number_of_exp})Attention_module_test_"+\
