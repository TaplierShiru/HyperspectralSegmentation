...
class NnModel(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        preds, cam_preds, pam_preds = self.model(img) # (N, C, 8, 8)
        # Calculate loss for each preds
        loss_preds = self.loss(preds, mask) # (N, 8, 8)
        loss_cam = self.loss(cam_preds, mask) # (N, 8, 8)
        ...
        loss_pam = self.bceloss(torch.sigmoid(pam_preds), target_one_hotted_tensor) # (N, 8, 8)


        loss = loss_preds + loss_cam * 0.4 + loss_pam * 0.2
...
class CAM_Module(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        energy = torch.bmm(proj_query, proj_key)
        energy_max_values = torch.max(energy, -1, keepdim=True)[0].expand_as(energy)
        energy_new = energy_max_values - energy     # In paper - this stuff was swapped, 
                                                    # i.e energy_max_values - energy
                                                    # but some guy, recommened to use: energy - energy_max_values
...
class MySuperNetLittleInput(nn.Module):
    ...
    def __init__(self, in_f=237, out_f=17, **kwargs):
        super().__init__()
        ...
        # Train branch for cam
        self.cam_train_output = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        )
        self.cam_final_output = nn.Conv2d(128, out_f, kernel_size=1, stride=1, padding=0, bias=False)
        # Output (N, 17, 8, 8)
        self.pam_module = PAM_Module(128, dim_reduse=8)
        self.pam_train_output = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        )
        self.pam_final_output = nn.Conv2d(128, out_f, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        cam_x = cam_x + x
        cam_x = self.cam_train_output(cam_x)

        pam_x = self.pam_module(x)
        pam_x = pam_x + x
        pam_x = self.pam_train_output(pam_x)
        ...
        if self.training:
            # Train phase, return several outputs
            cam_x = F.interpolate(cam_x, size=self.output_size)
            cam_output = self.cam_final_output(cam_x)

            pam_x = F.interpolate(pam_x, size=self.output_size)
            pam_output = self.pam_final_output(pam_x)
            return x, cam_output, pam_output
...
def return_net(arch_type: str, in_f: int, out_f: int, **kwargs):
    ...
    if arch_type == 'Unet':
        ...
        net = class_net(in_f, out_f, **kwargs)
    return net
...
def start_exp(
    ...
    net = return_net(arch_type=arch_type, in_f=17, out_f=17, output_size=cut_window)
    net.eval()
    with torch.no_grad():
    ...
    random_id = [random.randint(0, 1_000_000_000) for _ in range(100)][0]
    name_exp = f"Attention moduleV10 test cut_window={cut_window} (run {number_of_exp})" +\
