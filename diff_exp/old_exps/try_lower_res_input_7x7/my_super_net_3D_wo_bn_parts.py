...
NAME2CLASS = {
    MySuperNet3DLittleInput.__name__:             MySuperNet3DLittleInput,
}

ARCH_TYPES = [
    (MySuperNet3DLittleInput.__name__,            {"batch_size": 40}),
]
...
def start_exp(number_of_exp: int, T_0: int, T_mult: int, arch_type: str, batch_size: int):
    ...
    name_exp = f"MySuperNet3DLittleInput (run {number_of_exp}) | LrCosine W weight decay | lower arch | 70ep | Wo full PCA." +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})MySuperNet3DLittleInput | _LrCosine W weight decay lower_" +\
                f"arch_50ep_Wo full PCA._RustamPreprocess(k=1)_CE"+\
                f"cosine(t_0={T_0},t_mul={T_mult}) | arch_type={arch_type}",
        monitor="mean_iou",
