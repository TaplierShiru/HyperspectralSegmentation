...
def start_exp(number_of_exp: int):
    ...
    name_exp = f"Update data (run {number_of_exp})//lower arch//50ep//W PCA.//RustamPreprocess(k=1)//makiloss//gamma=5.5/balance=2"
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})Update data_lower arch_50ep_W PCA._RustamPreprocess(k=1)_makiloss_gamma=5.5_balance=2",
        monitor="mean_iou",
...
def main():
    for number_of_exp in range(1, N_REPEAT_EXP+1):
        start_exp(number_of_exp=number_of_exp)
