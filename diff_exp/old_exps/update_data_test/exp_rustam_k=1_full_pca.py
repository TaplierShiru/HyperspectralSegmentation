...
pca_explained_variance = np.load(f'new_full_PcaExplainedVariance_.npy')
pca_mean = np.load(f'new_full_PcaMean.npy')
pca_components = np.load(f'new_full_PcaComponents.npy')
...
def start_exp(number_of_exp: int):
    ...
    name_exp = f"Update data (run {number_of_exp})//lower arch//50ep//W full PCA.//RustamPreprocess(k=1)//makiloss//gamma=5.5/balance=2"
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})Update data_lower arch_50ep_W full PCA._RustamPreprocess(k=1)_makiloss_gamma=5.5_balance=2",
        monitor="mean_iou",
