...
class HsiDataloaderRudePreprocessMasks(torch.utils.data.IterableDataset):
    def __init__(
            self, 
            data_path: str,
        ...
        for idx, data_point in enumerate(self.dataset.data_iterator(opened=True, shuffle=False)):
            ...
            if cut_window is not None:
                preprocessed_mask = self._preprocess_mask_to_rude(mask, cut_window)
    ...
    def _preprocess_mask_to_rude(self, mask, cut_window):
        ...
        w_parts = w // w_win
        if h % h_win != 0:
            print(f"{h % h_win} pixels will be dropped by h axis. Input shape={mask.shape}")

        if w % w_win != 0:
            print(f"{w % w_win} pixels will be dropped by w axis. Input shape={mask.shape}")
        ...
        for h_i in range(h_parts):
            for w_i in range(w_parts):
                ...
                class_max = self.__get_class_w_max_pixels(mask_part)
    ...
    def __get_class_w_max_pixels(self, mask):
        max_class_id = -1
        max_pixel_count = -1
...
ARCH_TYPES = [
    (MySuperNet.__name__,                                       {"batch_size": 28}),
    (MySuperKernel3Net.__name__,                                {"batch_size": 28}),
]
...
def start_exp(number_of_exp: int, T_0: int, T_mult: int, arch_type: str, batch_size: int, gamma: float):
    ...
    dataset_train = HsiDataloaderRudePreprocessMasks(
        PATH_DATA, preprocessing=preprocessing, 
        augmentation=augmentation, indices=train_indices,
        shuffle_data=True
    )
    dataset_test = HsiDataloaderRudePreprocessMasks(
        PATH_DATA, preprocessing=preprocessing, 
    ...
    name_exp = f"MySuperNet with BN with rude masks (run {number_of_exp}) | LrCosine W weight decay | lower arch | 70ep | W full PCA." +\
    ...
    comet_exp.add_tag("exp_w_rude_masks")
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})MySuperNet with BN with rude masks | _LrCosine W weight decay lower_" +\
