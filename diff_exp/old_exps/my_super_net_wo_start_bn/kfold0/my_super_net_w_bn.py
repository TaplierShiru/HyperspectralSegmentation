...
def start_exp(number_of_exp: int, T_0: int, T_mult: int, arch_type: str, batch_size: int, gamma: float):
    ...
    name_exp = f"MySuperNet with BN (run {number_of_exp}) | LrCosine W weight decay | lower arch | 70ep | W full PCA." +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})MySuperNet with BN | _LrCosine W weight decay lower_" +\
