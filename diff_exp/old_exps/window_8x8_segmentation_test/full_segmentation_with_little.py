...
ARCH_TYPES = [
    (MySuperNetLittleInput.__name__,                            {"batch_size": 12}),
]
...
def start_exp(number_of_exp: int, T_0: int, T_mult: int, arch_type: str, batch_size: int, loss):
    ...
    net = return_net(arch_type=arch_type, in_f=237, out_f=17)
    ...
    name_exp = f"MySuperLittle with loss={str(loss)} (run {number_of_exp})//LrCosine W weight "+\
               f"decay//70ep//Wo full PCA." +\
               f"//RustamPreprocess(k=1)" +\
               f"cosine(t_0={T_0},t_mul={T_mult})//arch_type={arch_type}"
    comet_exp.set_name(name_exp)
    comet_exp.add_tag("Window_segm_vs_full_segm")

    model = NnModel(net, loss, experiment=comet_exp,
        T_0=T_0, T_mult=T_mult
    )

    # saves a checkpoint-file
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/MySuperLittle with loss={str(loss)} (run={number_of_exp})"+\
                f"_LrCosine Wo weight decay"+\
                f"_W0 full PCA._RustamPreprocess(k=1)__"+\
                f"cosine(t_0={T_0},t_mul={T_mult})//arch_type={arch_type}",
        monitor="mean_iou",
...
def main1(loss_init: callable):
    for arch_type, params in ARCH_TYPES:
        for number_of_exp in range(1, N_REPEAT_EXP+1):
            start_exp(
                number_of_exp=number_of_exp, T_0=T_0_param, loss=loss_init(),
                T_mult=T_mult_param, arch_type=arch_type, **params
            )

def main():
    for loss_type, loss_class in TRY_LOSSES.items():
        if loss_type == FOCAL_LOSS:
            # loop with gammas
            for gamma_val in GAMMAS_LIST:
                loss_init = lambda: loss_class(gamma=gamma_val)
                main1(loss_init=loss_init)
        else:
            loss_init = lambda: loss_class() # No extra params
            main1(loss_init=loss_init)
