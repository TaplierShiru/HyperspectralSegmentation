...
PREFIX_INFO_PATH = '/home/rustam/hyperspecter_segmentation/danil_cave/diff_exp/test_kfold/kfold0'
...
def start_exp(number_of_exp: int, T_0: int, T_mult: int, weight_decay: float):
    ...
    name_exp = f"kfold0-test(run {number_of_exp})//LrCosine W weight decay//lower arch//70ep//W full PCA." +\
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})kfold0-test_LrCosine W weight decay lower_" +\
                 "arch_50ep_W full PCA._RustamPreprocess(k=1)_makiloss_gamma=5.5_balance=2__"+\
...
def main():
    for number_of_exp in range(1, N_REPEAT_EXP+1):
        start_exp(
            ...
            T_mult=T_mult_param, weight_decay=weight_decay_param
        )
