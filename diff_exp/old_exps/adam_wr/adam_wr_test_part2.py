...
class NnModel(pl.LightningModule):
    def __init__(
            self, model, loss, weight_decay=0, 
...
pca_explained_variance = np.load(f'{PREFIX_INFO_PATH}/new_PcaExplainedVariance_.npy')
pca_mean = np.load(f'{PREFIX_INFO_PATH}/new_PcaMean.npy')
pca_components = np.load(f'{PREFIX_INFO_PATH}/new_PcaComponents.npy')
...
T_0_param_list = [10]
T_mult_param_list = [1, 2]
weight_decay_param_list = [0, 5e-4]


def start_exp(number_of_exp: int, T_0: int, T_mult: int, weight_decay: float):

    print(f'=======START EXP NUMBER {number_of_exp} ==============')
    print(f'Params: number_of_exp={number_of_exp}, T_0={T_0}, T_mult={T_mult}, weight_decay={weight_decay}')
    ...
    name_exp = f"LrCosine W weight decay (run {number_of_exp})//lower arch//50ep//W PCA." +\
                "//RustamPreprocess(k=1)//makiloss//gamma=5.5//balance=2" +\
                f"cosine(t_0={T_0},t_mul={T_mult})//weiht_decay={weight_decay}"
    comet_exp.set_name(name_exp)

    model = NnModel(net, FocalLossCustom(), experiment=comet_exp,
        weight_decay=weight_decay, T_0=T_0, T_mult=T_mult
    )

    # saves a checkpoint-file
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"pytorch_li_logs/(run={number_of_exp})LrCosine W weight decay lower_" +\
                 "arch_50ep_W PCA._RustamPreprocess(k=1)_makiloss_gamma=5.5_balance=2__"+\
                f"cosine(t_0={T_0},t_mul={T_mult})__weiht_decay={weight_decay}",
        monitor="mean_iou",
...
def main():
    for T_0 in T_0_param_list:
        for T_mult in T_mult_param_list:
            for weight_decay in weight_decay_param_list:
                for number_of_exp in range(1, N_REPEAT_EXP+1):
                    start_exp(
                        number_of_exp=number_of_exp, T_0=T_0,
                        T_mult=T_mult, weight_decay=weight_decay
                    )
